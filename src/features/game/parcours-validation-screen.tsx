...
export function ParcoursValidationScreen() {
  const validationData = useGameStore((s) => s.parcoursValidationData);
  const answerResults = useGameStore((s) => s.parcoursAnswerResults);
  const isHost = usePlayerStore((s) => s.isHost);
  const { validateParcoursAnswer } = useSocket();
  ...
  // Club timeline mini-display
  const ClubTimeline = () => (
    <div className="flex flex-wrap justify-center gap-1.5 mb-2">
      {validationData.clubs.map((club, i) => (
        <div
          key={i}
          className="px-2 py-1 rounded-lg bg-surface-800 border border-surface-700"
        >
          <span className="text-xs text-surface-300">{club.name}</span>
        </div>
      ))}
    </div>
  );
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 mt-auto">
                  <Button
                    ...
                    size="lg"
                    fullWidth
                    onClick={() => validateParcoursAnswer(currentPlayer.playerId, false)}
                  ...
                  <Button
                    ...
                    size="lg"
                    fullWidth
                    onClick={() => validateParcoursAnswer(currentPlayer.playerId, true)}
